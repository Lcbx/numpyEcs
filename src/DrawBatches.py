...
from RenderContext import GpuBuffer, Texture
...
class ShaderPass:
	...
	by visible_instances[instance_idx] in the current DrawData.entities order.
	"""
	pipeline: object
	bindings: tuple = ()
...
class DrawData:
	...
	mesh_rows: dict = field(default_factory=dict)
	dirty_meshes: set = field(default_factory=set)
	shaders: dict = field(default_factory=dict)
	buffers: dict = field(default_factory=dict)
	draw_batches: list = field(default_factory=list)
	entities: np.ndarray = field(default_factory=lambda: np.empty(0, dtype=np.uint64))
	batch_version: tuple = ()
	instance_version: tuple = ()
	dirty: bool = True
	bindings: dict = field(default_factory=dict)
	workgroup_count: int = 0
	prepass_uniform_buffer: object = None
	uniform_buffer: object = None
	camera_params_buffer: object = None
...
def create_draw_data(cull_shader):
	data = DrawData()
	...
	for name, dtype in (
		...
		data.buffers[name] = GpuBuffer(np.zeros(1, dtype=dtype), usage, upload=False, label=name)
	usage = wgpu.BufferUsage.UNIFORM | wgpu.BufferUsage.COPY_DST
	data.prepass_uniform_buffer = GpuBuffer(np.zeros(1, dtype=uniform_dtype), usage)
	data.uniform_buffer = GpuBuffer(np.zeros(1, dtype=uniform_dtype), usage)
	data.camera_params_buffer = cull_shader.UniformBuffer("camera_params")
	return data


def reserve_buffer(data, name, count):
...
def upload_array(data, name, values):
	reserve_buffer(data, name, len(values))[:] = values
	if len(values): data.buffers[name].upload_range(0, len(values))


def register_mesh(data, mesh_id, mesh, bounds=None):
	...
	data.meshes[mesh_id] = MeshInfo(mesh, center, extents)
	data.dirty_meshes.add(mesh_id)


def register_shader(data, shader_id, shader):
	...
	data.shaders[shader_id] = shader
	for name, spec, uniform, visible in (
		("prepass", shader.prepass, data.prepass_uniform_buffer, "frustum_visible_instances"),
		("main", shader.main, data.uniform_buffer, "main_visible_instances"),
	):
		data.bindings[shader_id, name] = (
			spec.pipeline.shader.bind_group(0, uniforms=uniform),
			spec.pipeline.shader.bind_group(1, instances=data.buffers["instances"], visible_instances=data.buffers[visible]),
		)


def sync_meshes(data):
	"""Refresh registered meshes before recording reset/culling commands."""
	for mesh_id in data.dirty_meshes:
		if mesh_id not in data.mesh_rows: continue
		info = data.meshes[mesh_id]
		row = data.mesh_rows[mesh_id]
		buffer = data.buffers["mesh_metadata"]
		...
		for batch in data.draw_batches:
			if batch.mesh_id != mesh_id: continue
			for name in ("prepass_draw_cmd", "main_draw_cmd"):
				buffer = data.buffers[name]
				command = buffer.content[batch.command_index]
				...
				buffer.upload_range(batch.command_index, 1)
	data.dirty_meshes.clear()
...
def sync_batches(data, world, transform_type, mesh_ref_type):
	"""Rebuild on membership or mesh/shader writes; return whether order changed.

	Custom per-instance buffers must follow data.entities after a rebuild.
	...
	if not data.dirty and data.batch_version == version:
		sync_meshes(data)
		return False
	...
	for batch in batches:
		if batch.mesh_id not in data.meshes: raise KeyError(f"Unregistered mesh_id {batch.mesh_id}")
		if batch.shader_id not in data.shaders: raise KeyError(f"Unregistered shader_id {batch.shader_id}")
	...
	for batch in batches:
		mesh = data.meshes[batch.mesh_id].mesh
	...
	for name in ("instances", "frustum_visible_instances", "main_visible_instances"):
		reserve_buffer(data, name, len(ordered))
	for name, values in (("mesh_metadata", metadata), ("batches", params), ("workgroups", workgroups), ("prepass_draw_cmd", commands), ("main_draw_cmd", commands)):
		upload_array(data, name, values)
	data.entities, data.draw_batches = ordered, batches
	data.mesh_rows = mesh_rows
	data.dirty_meshes.clear()
	data.workgroup_count = len(workgroups)
	data.batch_version = version
	data.instance_version = ()
	data.dirty = False
	return True


def resize_hzb(hzb, size, shader, pipelines):
	width, height = size
	if width <= 0 or height <= 0 or hzb.size == size: return False
	hzb.size = size
	usage = wgpu.TextureUsage.RENDER_ATTACHMENT | wgpu.TextureUsage.TEXTURE_BINDING
	hzb.depth_texture = Texture(size, format="depth32float", usage=usage, label="prepass_depth")
	width, height = max(1, width // 2), max(1, height // 2)
	num_mips = max(width, height).bit_length()
	usage = wgpu.TextureUsage.STORAGE_BINDING | wgpu.TextureUsage.TEXTURE_BINDING
	hzb.texture = Texture((width, height), format="r32float", usage=usage, mip_level_count=num_mips, label="hzb_pyramid")
	hzb.view = hzb.texture.view()
	views = [hzb.texture.view(base_mip_level=i, mip_level_count=1) for i in range(num_mips)]
	bindings = shader.bind_group(0, src_prepass=hzb.depth_texture.view(), dst_depth=views[0])
	hzb.passes = [(pipelines[0], bindings, width, height)]
	for mip in range(1, num_mips):
		width, height = max(1, width // 2), max(1, height // 2)
		bindings = shader.bind_group(0, src_depth=views[mip - 1], dst_depth=views[mip])
		hzb.passes.append((pipelines[1], bindings, width, height))
	return True


def refresh_bindings(data, cull_shader, hzb):
	buffers = data.buffers
	if "cull" not in data.bindings:
		...
		data.bindings["cull"] = cull_shader.bind_group(0, **common)
		data.bindings["frustum"] = cull_shader.bind_group(1, camera_params=data.camera_params_buffer)
		data.bindings["reset_prepass"] = cull_shader.bind_group(0, prepass_draw_cmd=buffers["prepass_draw_cmd"])
		data.bindings["reset_main"] = cull_shader.bind_group(1, camera_params=data.camera_params_buffer, main_draw_cmd=buffers["main_draw_cmd"])
	bindings = data.bindings.get("hiz")
	if bindings is None or bindings.resources["hzb_texture"] is not hzb.view:
		data.bindings["hiz"] = cull_shader.bind_group(1, camera_params=data.camera_params_buffer, hzb_texture=hzb.view, main_draw_cmd=buffers["main_draw_cmd"], main_visible_instances=buffers["main_visible_instances"])


def reset_draw_counts(cmd, data, pipeline):
	# Reset instance counters for indirect draw targets
	count = len(data.draw_batches)
	if not count: return
	groups = (count + 63) // 64
	x = min(groups, 65535)
	y = (groups + x - 1) // x
	if y > 65535: raise ValueError("Draw counter reset exceeds dispatch limits")
	with cmd.compute_pass(label="reset_draw_counts") as cp:
		cp.set_pipeline(pipeline)
		cp.set_bind_group(0, data.bindings["reset_prepass"])
		cp.set_bind_group(1, data.bindings["reset_main"])
		cp.dispatch(x, y)


def cull_instances(cmd, data, pipeline, stage):
	if not data.workgroup_count: return
	# Flatten a 2D dispatch so large scenes do not exceed the portable X limit.
	x = min(data.workgroup_count, 65535)
	y = (data.workgroup_count + x - 1) // x
	if y > 65535: raise ValueError("Culling workgroup table exceeds dispatch limits")
	with cmd.compute_pass(label=f"cull_{stage}") as cp:
		cp.set_pipeline(pipeline)
		cp.set_bind_group(0, data.bindings["cull"])
		cp.set_bind_group(1, data.bindings[stage])
		cp.dispatch(x, y)


def build_hzb(cmd, hzb):
	for mip, (pipeline, bindings, width, height) in enumerate(hzb.passes):
...
def draw_batches(rp, data, stage):
	commands = data.buffers[f"{stage}_draw_cmd"]
	for batch in data.draw_batches:
		spec = getattr(data.shaders[batch.shader_id], stage)
		rp.set_pipeline(spec.pipeline)
		for index, bindings in enumerate(data.bindings[batch.shader_id, stage]):
